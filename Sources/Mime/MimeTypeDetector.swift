...
public struct MimeType: Sendable {
  ...
  public func matches(bytes: [UInt8]) -> Bool {
    switch type {
      case .jpg: bytes.starts(with: [0xff, 0xd8, 0xff])
      case .png: bytes.starts(with: [0x89, 0x50, 0x4e, 0x47])
      case .gif: bytes.starts(with: [0x47, 0x49, 0x46])
      case .webp: bytes[8...11] == [0x57, 0x45, 0x42, 0x50]
      case .pdf: bytes.starts(with: [0x25, 0x50, 0x44, 0x46])
      case .zip: bytes.starts(with: [0x50, 0x4b, 0x03, 0x04])
      case .mp4: bytes[4...7] == [0x66, 0x74, 0x79, 0x70] // "ftyp"
      case .mp3: bytes.starts(with: [0x49, 0x44, 0x33]) || bytes.starts(with: [0xff, 0xfb])
      case .wav: bytes[8...11] == [0x57, 0x41, 0x56, 0x45]
      case .ogg: bytes.starts(with: [0x4f, 0x67, 0x67, 0x53])
      case .bz2: bytes.starts(with: [0x42, 0x5a, 0x68])
      case .rar: bytes.starts(with: [0x52, 0x61, 0x72, 0x21, 0x1a, 0x07])
      case .tar: bytes[257...261] == [0x75, 0x73, 0x74, 0x61, 0x72]
      case .mov: bytes.starts(with: [0x00, 0x00, 0x00, 0x14, 0x66, 0x74, 0x79, 0x70])
      case .flac: bytes.starts(with: [0x66, 0x4c, 0x61, 0x43])
      case .tif: (bytes.starts(with: [0x49, 0x49, 0x2a, 0x00]) || bytes.starts(with: [0x4d, 0x4d, 0x00, 0x2a]))
      case .avi: (bytes.starts(with: [0x52, 0x49, 0x46, 0x46]) && bytes[8...10] == [0x41, 0x56, 0x49])
      case .wmv: bytes.starts(with: [0x30, 0x26, 0xb2, 0x75])
      case .psd: bytes.starts(with: [0x38, 0x42, 0x50, 0x53])
      case .exe: bytes.starts(with: [0x4d, 0x5a])
      case .xz: bytes.starts(with: [0xfd, 0x37, 0x7a, 0x58])
      case .flv: bytes.starts(with: [0x46, 0x4c, 0x56, 0x01])
      default: false
    }
  }
...
public struct MimeTypeDetector: Sendable {
  ...
  public static func detectMimeType(fromExtension fileExtension: String) -> MimeType? {
    MimeType.all.first { $0.ext == fileExtension }
  }
}
